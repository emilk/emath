...
namespace emath
{
	...
	namespace trace
	{
		...
		bool ray_circle(info& ti, const Circle& c)
		{
			...
			// x^2 * dir.length_sq() + x*2*dot(dir,R) + R.length_sq() - Sqr(r) = 0
		...
		bool ray_capsule(info& ti, const CapsuleBaked& cap)
		{
			...
			if (!is_zero(d.y)) {
				...
				if (s < 0) {
					...
					assert(cap.line_seg.is_approachedBy(ray));
		...
		bool ray_circleCCD(info& ti, const Circle& c_0, const Circle& c_1)
		{
			 ...
			 */
			real x = solve_X2_X_C(a, b, c, [=](real x0, real x1){return sign*max(sign*x0, sign*x1);});

			if (std::isfinite(x) && ti.min_t < x && x < ti.t) {
				ti.t = x;
