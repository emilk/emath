...
namespace emath
{
	...
	const real F2 = real( 0.5*(std::sqrt(3.0) - 1.0)  );
	const real G2 = real( (3.0-std::sqrt(3.0))/6.0    );
	const real F3 = real( 1.0/3.0                     );
	const real G3 = real( 1.0/6.0                     );
	const real F4 = real( (std::sqrt(5.0)-1.0)/4.0    );
	const real G4 = real( (5.0-std::sqrt(5.0))/20.0   );

	int fastfloor(real x) {
		//int xi = (int)x;
		//return x<xi ? xi-1 : xi;
		return (int)std::floor(x);
	}

	real dot(Grad2 g, real x, real y) {
		return g.x*x + g.y*y; }

	real dot(Grad3 g, real x, real y, real z) {
		return g.x*x + g.y*y + g.z*z; }

	real dot(Grad4 g, real x, real y, real z, real w) {
		return g.x*x + g.y*y + g.z*z + g.w*w; }

	/*
	dot(Grad3 g, real x, real y, real z, real w) {
		return g.x*x + g.y*y + g.z*z + g.w*w;
	}
	*/
	...
	real noise_2d(real xin, real yin) {
		real n0, n1, n2; // Noise contributions from the three corners
		// Skew the input space to determine which simplex cell we're in
		real s = (xin+yin)*F2; // Hairy factor for 2D
		int i = fastfloor(xin+s);
		int j = fastfloor(yin+s);
		real t = (i+j)*G2;
		real X0 = i-t; // Unskew the cell origin back to (x,y) space
		real Y0 = j-t;
		real x0 = xin-X0; // The x,y distances from the cell origin
		real y0 = yin-Y0;
	...
	real noise_3d(real xin, real yin, real zin)
	{
		...
		real t0 = 0.6f - x0*x0 - y0*y0 - z0*z0;
		if (t0<0) n0 = 0.0;
		else {
			t0 *= t0;
			n0 = t0 * t0 * dot(grad3[gi0], x0, y0, z0);
		}
		real t1 = 0.6f - x1*x1 - y1*y1 - z1*z1;
		if (t1<0) n1 = 0.0;
		else {
			t1 *= t1;
			n1 = t1 * t1 * dot(grad3[gi1], x1, y1, z1);
		}
		real t2 = 0.6f - x2*x2 - y2*y2 - z2*z2;
		if (t2<0) n2 = 0.0;
		else {
			t2 *= t2;
			n2 = t2 * t2 * dot(grad3[gi2], x2, y2, z2);
		}
		real t3 = 0.6f - x3*x3 - y3*y3 - z3*z3;
		if (t3<0) n3 = 0.0;
		else {
			t3 *= t3;
	...
	real noise_4d(real x, real y, real z, real w) {
		...
		real s = (x + y + z + w) * F4; // Factor for 4D skewing
		int i = fastfloor(x + s);
		int j = fastfloor(y + s);
		int k = fastfloor(z + s);
		int l = fastfloor(w + s);
		...
		real Z0 = k - t;
		real W0 = l - t;
		real x0 = x - X0;  // The x,y,z,w distances from the cell origin
		real y0 = y - Y0;
		real z0 = z - Z0;
		real w0 = w - W0;
		...
		real t0 = 0.6f - x0*x0 - y0*y0 - z0*z0 - w0*w0;
		if (t0<0) n0 = 0.0;
		else {
			t0 *= t0;
			n0 = t0 * t0 * dot(grad4[gi0], x0, y0, z0, w0);
		}
		real t1 = 0.6f - x1*x1 - y1*y1 - z1*z1 - w1*w1;
		if (t1<0) n1 = 0.0;
		else {
			t1 *= t1;
			n1 = t1 * t1 * dot(grad4[gi1], x1, y1, z1, w1);
		}
		real t2 = 0.6f - x2*x2 - y2*y2 - z2*z2 - w2*w2;
		if (t2<0) n2 = 0.0;
		else {
			t2 *= t2;
			n2 = t2 * t2 * dot(grad4[gi2], x2, y2, z2, w2);
		}
		real t3 = 0.6f - x3*x3 - y3*y3 - z3*z3 - w3*w3;
		if (t3<0) n3 = 0.0;
		else {
			t3 *= t3;
			n3 = t3 * t3 * dot(grad4[gi3], x3, y3, z3, w3);
		}
		real t4 = 0.6f - x4*x4 - y4*y4 - z4*z4 - w4*w4;
		if (t4<0) n4 = 0.0;
		else {
			t4 *= t4;
